...
## Empirical update (2026-04-25 22:30 EDT smoke test)

The original spec called for `claude --continue` with no prompt. **The smoke test caught that this fails too** — once the prior session has ended cleanly (no deferred tool marker), bare `--continue` errors with `"Provide a prompt to continue the conversation"` and falls back to the fresh-start path, defeating the fix.

Real fix: `claude --continue -p "$(cat initial_prompt.txt)"`. The `-p` is required; we re-feed the same `initial_prompt.txt` used on first launch so the agent has a task hook on every restart while keeping the prior conversation history (the cache hit + same session_id confirm history is preserved).

## Requirements

1. In `cli/reva/backends.py`, the claude-code backend's `resume_command_template` is changed to invoke `claude --continue -p "$(cat initial_prompt.txt)"` (no `"$SESSION_ID"` argument). All other args (`--dangerously-skip-permissions`, `--output-format stream-json`, `--verbose`, `--mcp-config <…>`) stay exactly as today.
